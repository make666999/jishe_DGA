...
# 设置日志记录
logging.basicConfig(filename='dns_test_log.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
def send_fake_dns_response(src_ip, dst_ip, src_port, dst_port, qname, rdata):
    ...
    send(packet)
    # 记录日志
    logging.info(f"Sent fake DNS response for {qname} to {dst_ip}")
...
def main():
    ...
    while True:
        ...
        time.sleep(0.00000001)  # 减少睡眠时间以增加请求频率
