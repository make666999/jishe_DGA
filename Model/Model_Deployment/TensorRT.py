...
if __name__ == '__main__':
    ...
    input = torch.randint(0, 30, (256, 40)).float().cuda()
    trt_mod = torch_tensorrt.compile(model, ir="dynamo", inputs=[input],enabled_precisions={torch.half})
    out_put = trt_mod(input)
    print(out_put)
    test_quantized(trt_mod, test_loader)